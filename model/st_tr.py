...
import math
import tqdm
from .unit_gcn1 import unit_gcn1
from .unit_agcn import unit_agcn

from .gcn_attention import gcn_unit_attention
from .net import conv_init, Unit2D
from .ctrgcn import MultiScale_TemporalConv, import_class, unit_gcn, unit_tcn
...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        unit = TCN_GCN_unit #Transformer-gcn
        ...
        if not all_layers:
            ...
            self.tcn0 = MultiScale_TemporalConv(backbone_in_c, backbone_in_c, kernel_size=9)
    ...
    def forward(self, x, label, name):
        ...
        # model
        if not self.all_layers: #ctr-gcn
            x = self.gcn0(x, label, name)
            x = self.tcn0(x)

        for i, m in enumerate(self.backbone):
            if (i == 4) and self.concat_original:
...
class TCN_GCN_unit(nn.Module):
    def __init__(self,in_channel,out_channel,A,attention,only_attention,relative,device,
                 attention_3,dv,dk, Nh,num,dim_block1,dim_block2,dim_block3,num_point,weight_matrix,more_channels, drop_connect,
                 starting_ch, all_layers,adjacency, data_normalization, visualization, skip_conn, layer=0, kernel_size=9, stride=1,
                 dropout=0.5, use_local_bn=False, mask_learning=False, last=False,last_graph=False, agcn=False ):
